...
struct MemoryEditor
{
    ...
    void DrawWindow(const char* title, void* mem_data, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        ...
        Open = true;
        if (ImGui::Begin(title, &Open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_MenuBar))
        {
            if (ImGui::BeginMenuBar()) {
                if (ImGui::BeginMenu("View")) {
                    ImGui::InputText("Name", TempTitle, sizeof(TempTitle));
                    if (ImGui::Button("Apply")) { strncpy(Title, TempTitle, sizeof(Title)); }
